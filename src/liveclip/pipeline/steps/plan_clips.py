...
PROHIBITED_PROMOTION_PATTERNS: tuple[re.Pattern[str], ...] = tuple(
    re.compile(pattern)
    for pattern in (
        r"(免费|免[费废]的?|0\s*元|零元|一毛钱不需要).{0,12}(领取|领|试用|体验|名额|粉丝群|进群|关注|带回去|教程)",
        r"(领取|领|进群|粉丝群|关注|名额).{0,12}(免费|免[费废]的?|试用|体验|七天|7\s*天)",
        r"(七天|7\s*天).{0,12}(免费|免[费废]的?|试用|体验|领取|领|用)",
        r"(免费|免[费废]的?).{0,6}(小关小注|关注)",
        r"(免费用|免费试用|免费体验|免费领取|领取教程|领取我们的|领取七天|试用名额|免费名额)",
        r"(还没|没有|没|未).{0,8}领到.{0,12}(试用|体验|名额|AI智能体)?",
        r"(教|讲|说).{0,8}(怎么|如何).{0,8}(领取|领)",
    )
)
...
def parse_llm_response(
    ...
    for i, item in enumerate(segments_data):
        ...
        try:
            ...
            content_score = clamp_score(float(raw_score)) if raw_score not in (None, "") else 0.0
            structure_score = clamp_score(float(item.get("structure_score", 0.0)))
            if "structure_score" in item and structure_score < 0.4:
                continue
            score = content_score
            if structure_score:
...
def contains_prohibited_promotion(text: str) -> bool:
    """Return true when text is mainly a platform-risky promotion/claim CTA."""
    compact = re.sub(r"\s+", "", text)
    if not compact:
        return False
    return any(pattern.search(compact) for pattern in PROHIBITED_PROMOTION_PATTERNS)


def _segment_text(segment: ClipSegment, subtitles: list[SubtitleEntry]) -> str:
    if segment.parts:
        chunks: list[str] = []
        for part in segment.parts:
            ...
            if start_value is None or end_value is None:
                continue
            chunks.extend(row.text for row in subtitles if start_value <= row.index <= end_value)
        return "".join(chunks)
    return "".join(
        row.text
        for row in subtitles
...
def is_prohibited_promotion_segment(
    segment: ClipSegment,
    subtitles: list[SubtitleEntry],
) -> bool:
    """Filter clips centered on free-trial/claim/join-group inducements."""
    fields = [
        segment.title,
        segment.reason,
        segment.structure_reason,
        segment.hook,
        _segment_text(segment, subtitles),
    ]
    return any(contains_prohibited_promotion(field) for field in fields)
...
def postprocess_segments(
    ...
    before_policy_count = len(filtered)
    filtered = [seg for seg in filtered if not is_prohibited_promotion_segment(seg, subtitles)]
    logger.info(
        "segments_after_prohibited_promotion_filter",
        run_id=ctx.run_id,
        input_count=before_policy_count,
